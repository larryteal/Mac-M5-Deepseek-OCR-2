...
func ProcessImage(ctx ml.Context, bts []byte) (ml.Tensor, ml.Tensor, []int, error) {
	...
	// maxNum=6 matches the Python reference (dynamic_preprocess default)
	minNum, maxNum, imageSize, baseSize := 2, 6, 768, 1024
	...
	// DeepSeek-OCR-2 uses [0.5, 0.5, 0.5] normalization (maps to [-1, 1])
	// NOT ImageNet standard values. See processor_config.json.
	...
	// Pad to maxNum blocks to ensure consistent compute graph dimensions.
	// The ggml Metal backend requires the same tensor shapes between the
	// probe (during model loading) and actual inference to avoid residency
	// set assertion failures.
	realBlocks := blocks
	patchSize := imageSize * imageSize * 3
	for blocks < maxNum {
		patches = append(patches, make([]float32, patchSize)...)
		blocks++
	}

	// v2: keep RGB (v1 converted to grayscale which is wrong for v2)
	...
	return ctx.Input().FromFloats(patches, imageSize, imageSize, 3, blocks),
		...
		[]int{targetRatio.x, targetRatio.y, realBlocks},
		nil
}
